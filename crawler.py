...
VEHICLE_NUMBERS = [
    ...
    '3910', '6078', '7986', '9577',
    '1477',  # 106하1477 E200 AMG Line: 2026-08-27 운행시작, 9/1부터 신동석(jiip) — OWNER_TRANSFERS 참조 (사용자 지정 2026-09-07)
    # 2026-08-24 사용자 지시로 제외: 5080(이력도 삭제), 9578(이력은 hq 유지), 5125(당일 추가분 철회, 이력 없음)
    # (8/24 지시에서 9577↔9578 이 뒤바뀌어 있었음 — 8/25 정정: 남는 쪽=9577, 빠지는 쪽=9578)
]
...
KIM_VEHICLES = [
    ...
    '8681',  # 106호8681 레이 프레스티지: 김민규 신규 — 5438 모닝 반납 후 대체 (사용자 확인 2026-09-07)
    '9588',  # 115하9588 420i 컨버터블: 김민규 신규, 모빌리티(법인2) 운행시작 2026-08-21 (사용자 확인 2026-09-07)
    # 2026-09-07 제외: 5438 모닝 — 김민규 반납 → 본사 정비중 (BL매니저 8/31 사고대여 해제). 이력 0건, fleet 행 삭제.
]

# 박민 차량 18대 (owner='park' 로 태그) — 사용자 모드 코드 '0002'
# 출처: blrent-car-system vehicles.customer_name='박민소장' (2026-08-03 정본 갱신).
# ⚠ 9315 는 vehicles 상 박민이나 김민규(kim) 유지하기로 함(사용자 지정) → 여기서 제외.
# ⚠ 박민정(106호8003)은 다른 사람이라 제외.
PARKMIN_VEHICLES = [
    '8432', '4072', '9246', '9247', '9418',
    '9454', '9455', '9497', '9521', '9544',
    '9660', '9758', '9791', '9836', '9880',
    '9891', '9892', '2727',
    '1773',  # 106호1773 G80: 박민 신규 (2026-08-24 추가)
    # 매각 차량 (BL매니저 정본, 사용자 확인 2026-09-07): 9791 X3 7/14, 1773 G80 8/26, 4072 S580 8/26.
    # 미입금 건(4072 8/6~8/22 청구완료 등) 입금 추적을 위해 검색 목록엔 유지. fleet status='sold' 로 표시.
]

# 굿초이스 차량 23대 (owner='good' 로 태그) — 사용자 모드 코드 '0003'
# 출처: blrent-car-system vehicles.customer_name LIKE '%굿초이스%' (2026-08-03 정본 갱신).
GOODCHOICE_VEHICLES = [
    '8214', '8263', '8264', '2209', '3268',
    '3392', '3598', '3599', '3620', '3652',
    '3720', '3858', '3923', '4352', '4472',
    '5081', '5152', '5154', '5745', '6098',
    '6448', '6641', '8679',
]
...
# 전상민 차량 (owner='jeon' 로 태그) — 사용자 모드 코드 '0006'. 기존 3대(7898·7899·7951)는 KIM_VEHICLES + OWNER_TRANSFERS 로 이관된 것.
JEON_VEHICLES = [
    '9587',  # 115하9587 BMW Z4: 전상민 신규, 모빌리티(법인2) 운행시작 2026-08-21 (사용자 확인 2026-09-07)
]
...
OWNER_TRANSFERS = {
    ...
    '7940': [('2026-07-01', 'jiip'), ('2026-08-15', 'choi')],  # 106호7940 GLE 쿠페 → 최제렬 (8/15부터 — 사용자 지정 2026-08-24)
    '9894': [('2026-07-01', 'jiip'), ('2026-08-01', 'kim')],   # 106호9894 E 클래스 → 김민규 (8/1부터 — 사용자 지정 2026-08-24)
    # 본사 → 굿초이스 (2026-08-24 지시, 8/25 9577↔9578 정정): 115하9577 스포티지 HEV, 8/20 이관.
    # 이전 내역은 hq 유지, fleet 미등록("내역만 본사"). BL매니저 vehicles(9577=굿초이스렌터카)와 일치.
    '9577': [('2026-08-20', 'good')],
    # 본사 → 신동석 (사용자 지정 2026-09-07): 106하1477 E200 AMG Line, 9/1 이후 시작 건부터 jiip. 신규 차량이라 이전 내역 없음(있으면 hq).
    '1477': [('2026-09-01', 'jiip')],
    # 신동석(jiip) → 장명재 — 7/1 이후 시작 건부터 (사용자 지정 2026-08-25, 당초 8/24에서 소급)
    '4286': [('2026-07-01', 'jang')], '8433': [('2026-07-01', 'jang')],
    '9256': [('2026-07-01', 'jang')],
    # 106호9194 머스탱: 신동석 → 장명재(7/1) → 김민규(9/1 이후 시작 건부터 — 사용자 지정 2026-09-07)
    '9194': [('2026-07-01', 'jang'), ('2026-09-01', 'kim')],
    '9334': [('2026-07-01', 'jang')], '9340': [('2026-07-01', 'jang')],
    '9341': [('2026-07-01', 'jang')], '9388': [('2026-07-01', 'jang')],
    '9433': [('2026-07-01', 'jang')], '9558': [('2026-07-01', 'jang')],
    '9666': [('2026-07-01', 'jang')], '9754': [('2026-07-01', 'jang')],
    '9755': [('2026-07-01', 'jang')], '9759': [('2026-07-01', 'jang')],
    '9878': [('2026-07-01', 'jang')], '9893': [('2026-07-01', 'jang')],
    # 신동석(jiip) → 강연수 — 4/1 이후 시작 건부터 (사용자 지정 2026-08-25, 당초 8/24에서 소급)
    '9895': [('2026-04-01', 'kang')],
    # 김민규(kim) → 최제렬 — 4/1 이후 시작 건부터 (사용자 지정 2026-08-25, 당초 8/24에서 소급)
    # ⚠ 7940(8/15)·7950·9879 는 본사/신동석 이관 이력이 있어 소급 제외 (위 개별 항목 유지)
    '7891': [('2026-04-01', 'choi')], '9116': [('2026-04-01', 'choi')],
    '9137': [('2026-04-01', 'choi')], '9283': [('2026-04-01', 'choi')],
    '9315': [('2026-04-01', 'choi')], '9756': [('2026-04-01', 'choi')],
    '9774': [('2026-04-01', 'choi')],
    # 김민규(kim) → 전상민 — 4/1 이후 시작 건부터 (사용자 지정 2026-08-25)
    '7898': [('2026-04-01', 'jeon')], '7899': [('2026-04-01', 'jeon')],
    '7951': [('2026-04-01', 'jeon')],
    # 김민규(kim) → 유윤빈 — 4/1 이후 시작 건부터 (사용자 지정 2026-08-25)
    '7892': [('2026-04-01', 'yu')], '9646': [('2026-04-01', 'yu')],
}
